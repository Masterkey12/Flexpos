...
<div class="modal-header">
    ...
    <h4 class="modal-title">@if(!empty($account)) {{__('Edit Account')}} @else {{__('Ajouter un compte')}}@endif</h4>
</div>
<div class="modal-body" >
    <div class="row">
        <div class="col-md-6" >
            <div class="form-group row">
                {{ Form::label('name', __('Nom') .' *',['class'=>'col-sm-3 text-right']) }}
                <div class="col-sm-9">
                    {{ Form::text('name', null, array('class' => 'form-control', 'required')) }}
                </div>
            </div>
            <div class="form-group row">
                {{ Form::label('company', __('Banque').' *',['class'=>'col-sm-3 text-right']) }}
                <div class="col-sm-9">
                    {{ Form::text('company', null, array('class' => 'form-control', 'required')) }}
                </div>
            </div>
            <div class="form-group row">
                {{ Form::label('branch_name', __('Branche'), ['class'=>'col-sm-3 text-right']) }}
                <div class="col-sm-9">
                    {{ Form::text('branch_name', null, array('class' => 'form-control')) }}
                </div>
            </div>
            <div class="form-group row">
                {{ Form::label('account_no', __('N° de compte'),['class'=>'col-sm-3 text-right']) }}
                <div class="col-sm-9">
