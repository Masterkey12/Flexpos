...
class Sale extends Model
{
    ...
    public function getStatus()
    {
        $status = '';
        if ($this->status == self::HOLD) {
            $status = '<span class="label label-danger">Tenue</span>';
        } else if ($this->status == self::DUE) {
            $status = '<span class="label label-warning">Dû</span>';
        } else if ($this->status == self::PAID) {
            $status = '<span class="label label-success">Payé</span>';
        } else if ($this->status == self::REFUND) {
            $status = '<span class="label label-danger">Remboursé</span>';
        }
        return $status;
    }
